...
class config(AttrDisplay):
    def __init__(self):
        self.predict_T = 3
        self.use_take = True
        self.new_length = 2*(16 + self.predict_T)
        self.new_step = 1#2 if not self.use_take else 1
        ...
        self.save_dir = 'logs/param_rgb_r2plus1d_resnet18_kinetics400_custom_hmdb51_nlength32_lateral_scratch_tanh_T_realPreTAKE_nowarp'
        ...
        for _, X in enumerate(data):
            if opt.reshape_type == 'tsn':
                if opt.predict_T == 0:
                    ...
                    #X = X.take(nd.array([2*n for n in range(16)]).as_in_context(ctx),axis=3)
                    #X = X.take(nd.arange(0,opt.new_length,2).as_in_context(ctx),axis=3)
                    X = takeT(X)

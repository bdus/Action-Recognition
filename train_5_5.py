...
class config(AttrDisplay):
    def __init__(self):
        self.predict_T = 1
        self.new_length = 16 + self.predict_T
        self.new_step = 2
        ...
        self.save_dir = 'logs/param_rgb_r2plus1d_resnet18_kinetics400_custom_hmdb51_nlength16_lateral_scratch_tanh_T_eq1'
        ...
        self.use_pretrained = False #False
        ...
        self.resume_path = 'logs/param_rgb_r2plus1d_resnet18_kinetics400_custom_hmdb51_nlength16_lateral_scratch_tanh'
        self.resume_params = os.path.join(self.resume_path,'0.1712-hmdb51-r2plus1d_resnet18_kinetics400_custom-022-best.params')
        self.resume_states = os.path.join(self.resume_path,'0.1712-hmdb51-r2plus1d_resnet18_kinetics400_custom-022-best.states')
...
train_history_loss = TrainingHistory(['training-loss','cross-loss','mse-loss','pre-loss'])
lr_decay_count = 0
best_val_score = 0
    ...
    for i, batch in enumerate(val_data):
        ...
        for _, X in enumerate(data):
            if opt.reshape_type == 'tsn':
                if opt.predict_T == 0:
                    ...
                    #X2 = X.astype(opt.dtype, copy=False).slice_axis(axis=3, begin=opt.predict_T, end=opt.new_length)
                    X = X.reshape((-1,) + X.shape[2:]) # for reconstraction
        ...
        if not opt.use_lateral:
            ...
            if opt.predict_T == 0: # normal
                ...
                loss0 = [loss_fn(yhat, y.astype(opt.dtype, copy=False)) for yhat, y in zip(output, label)]
                loss1 = [loss_l2(xhat,x.astype(opt.dtype, copy=False).slice_axis(axis=3, begin=0, end=(opt.new_length - opt.predict_T) ) ) for xhat, x in zip(l2out,data )]
                loss2 = [loss_l2(xhat,x.astype(opt.dtype, copy=False).slice_axis(axis=3, begin=opt.predict_T, end=opt.new_length) ) for xhat, x in zip(l2out,data )]
                loss = [(l1+l2+l3) for l1, l2, l3 in zip(loss0,loss1,loss2)]
        ...
        val_loss_epoch2 += sum([l.mean().asscalar() for l in loss1]) / len(loss2)
    
    _, top1 = acc_top1.get()
    _, top5 = acc_top5.get()
    val_loss = val_loss_epoch / num_test_iter
    loss_mse = val_loss_epoch1 / num_test_iter
    loss_pre = val_loss_epoch2 / num_test_iter    
    
    return (top1, top5, val_loss, loss_mse,loss_pre)
...
for epoch in range(opt.resume_epoch, opt.epochs):
    ...
    for i, batch in enumerate(train_data):
        ...
        pre_loss  += sum([l.mean().asscalar() for l in loss1]) / len(loss2)
        
        train_metric.update(label, output)
        if i % opt.log_interval == 0:
            name, acc = train_metric.get()
            logger.info('[Epoch %d] [%d | %d] train=%f loss=%f mseloss=%f  pre_loss %f time: %f' %
                  (epoch,i,len(train_data), acc, train_loss / (i+1),mse_loss/(i+1),pre_loss/(i+1), time.time()-btic) )
            btic = time.time()
    ...
    acc_top1_val, acc_top5_val, loss_val, loss_mse, loss_pre = test(ctx, val_data) 
    ...
    train_history_loss.update([train_loss/(i+1),loss_val,loss_mse,loss_pre])
    train_history_loss.plot(save_path=os.path.join(opt.save_dir,'trainlog_loss.jpg'))
    ...
    logger.info('[Epoch %d] val top1 =%f top5=%f val loss=%f,mesloss=%f,loss_pre = %f, lr=%f' %
        (epoch, acc_top1_val, acc_top5_val, loss_val ,loss_mse,loss_pre,trainer.learning_rate ))

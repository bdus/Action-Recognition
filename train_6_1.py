...
class config(AttrDisplay):
    def __init__(self):
        ...
        self.save_dir = 'logs/param_rgb_r2plus1d_resnet18_kinetics400_custom_hmdb51_nlength16_lateral_scratch_tanh_pre_train61'
        ...
        self.use_amp = False
        self.epochs = 160
        self.lr_decay_epoch = [60,90]
        self.lr_decay_period = 0
        self.scale_ratios = [1.0, 0.8]#[1.0, 0.875, 0.75, 0.66]
        self.dtype = 'float32'
        self.pretrained_lateral_path = 'logs/param_rgb_r2plus1d_resnet18_kinetics400_custom_hmdb51_nlength16_lateral_scratch_tanh_T_eq1'
        self.pretrained_lateral_file = '0.3758-hmdb51-r2plus1d_resnet34_tranconv_lateral_tanhbn-077-best.params' 
    ...
    for i, batch in enumerate(val_data):
        ...
        for _, X in enumerate(data):
            if opt.reshape_type == 'tsn':
                ...
                elif opt.predict_T > 0:
                    #X = X.slice_axis(axis=3, begin=0, end=(opt.new_length - opt.predict_T))
                    X1 = takeT(X)
            ...
            if not opt.use_lateral:
                ...
                val_l2out.append(x_hat) 
                val_l2latel.append(latel)
                val_l2latel1.append(latel1)
            #pred = net(X.astype(opt.dtype, copy=False))
        ...
        if not opt.use_lateral:
            ...
            if opt.predict_T == 0: # normal
                loss0 = [loss_fn(yhat, y.astype(opt.dtype, copy=False)) for yhat, y in zip(val_outputs, label)]
                loss1 = [loss_l2(xhat,x.astype(opt.dtype, copy=False)) for xhat, x in zip(val_l2out,data )]
                loss = [(l1+l2) for l1, l2 in zip(loss0,loss1)]
            elif opt.predict_T > 0: # predict
                loss0 = [loss_fn(yhat, y.astype(opt.dtype, copy=False)) for yhat, y in zip(val_outputs, label)]
                loss3 = [loss_l2(xhat, takeT(x.astype(opt.dtype, copy=False)) )  for xhat, x in zip(val_l2out,data )]
                loss1 = [loss_l2(xhat, takeT(x.astype(opt.dtype, copy=False),T=opt.predict_T)) for xhat, x in zip(val_l2out,data )]                
                #loss2 = [loss_l2(f1,f2) for f1, f2 in zip(val_l2latel,val_l2latel1) ]
                loss2 = [(loss_l2(f1[2],f2[2]) + loss_l2(f1[1],f2[1]) ) for f1, f2 in zip(l2latel,l2latel1) ]
                loss = [(l0+l1+l2+l3) for l0, l1, l2, l3 in zip(loss0,loss1,loss2,loss3)]
        ...
        val_loss_epoch2 += sum([l.mean().asscalar() for l in loss2]) / len(loss2)
        val_loss_epoch3 += sum([l.mean().asscalar() for l in loss3]) / len(loss3)
    
    _, top1 = acc_top1.get()
    _, top5 = acc_top5.get()
    val_loss = val_loss_epoch / num_test_iter
    loss_mse = val_loss_epoch1 / num_test_iter
    loss_pre = val_loss_epoch3 / num_test_iter
    loss_flow = val_loss_epoch2 / num_test_iter
    
    return (top1, top5, val_loss, loss_mse,loss_pre,loss_flow)
...
for epoch in range(opt.resume_epoch, opt.epochs):
    ...
    for i, batch in enumerate(train_data):
        ...
        with ag.record():
            ...
            if not opt.use_lateral:
                ...
                if opt.predict_T == 0: # normal
                    ...
                    loss0 = [loss_fn(yhat, y.astype(opt.dtype, copy=False)) for yhat, y in zip(output, label)]                                       
                    loss2 = [loss_l2(xhat, takeT(x.astype(opt.dtype, copy=False)) )  for xhat, x in zip(l2out,data )]
                    loss1 = [loss_l2(xhat, takeT(x.astype(opt.dtype, copy=False),T=opt.predict_T)) for xhat, x in zip(l2out,data )]                    
                    loss3 = [(loss_l2(f1[2],f2[2]) + loss_l2(f1[1],f2[1]) ) for f1, f2 in zip(l2latel,l2latel1) ]
                    loss = [(l0+l1+l2+l3) for l0, l1, l2, l3 in zip(loss0,loss1,loss2,loss3)]
        ...
        pre_loss  += sum([l.mean().asscalar() for l in loss2]) / len(loss2)
        flow_loss  += sum([l.mean().asscalar() for l in loss3]) / len(loss3)
        
        train_metric.update(label, output)
        if i % opt.log_interval == 0:
            name, acc = train_metric.get()
            logger.info('[Epoch %d] [%d | %d] train=%f loss=%f mseloss=%f  pre_loss %f flow: %f time: %f' %
                  (epoch,i,len(train_data), acc, train_loss / (i+1),mse_loss/(i+1),pre_loss/(i+1),flow_loss/(i+1) , time.time()-btic) )
            btic = time.time()
    ...
    train_history.update([acc,acc_top1_val,acc_top5_val,train_loss/(i+1),loss_val,loss_mse,loss_pre])
    ...
    logger.info('[Epoch %d] val top1 =%f top5=%f val loss=%f,mesloss=%f,loss_pre = %f,loss_flow=%f, lr=%f' %
        (epoch, acc_top1_val, acc_top5_val, loss_val ,loss_mse,loss_pre,lossflow,trainer.learning_rate ))

...
__all__ = ['r2plus1d_resnet18_kinetics400_custom','r2plus1d_resnet34_kinetics400_custom','r2plus1d_resnet34_tranconv_lateral','r2plus1d_resnet34_tranconv_lateral_tanhbn','r2plus1d_resnet18_aetfc']
...
class R2Plus1D_AET2FC(HybridBlock):
    def __init__(self, dropout_ratio=0.5,expansion=1,nclass=4,
                 num_segments=1, feat_ext=False,
                 init_std=0.001, ctx=None, partial_bn=False,
                 norm_layer=BatchNorm, norm_kwargs=None, **kwargs):
        super(R2Plus1D_AET2FC, self).__init__()
        self.partial_bn = partial_bn
        self.dropout_ratio = dropout_ratio
        self.init_std = init_std
        self.num_segments = num_segments        
        ...
        self.feat_dim = 512 
        with self.name_scope():
            #self.bn1 = norm_layer(in_channels=256, **({} if norm_kwargs is None else norm_kwargs))
            self.relu = nn.Activation('relu')            
            #self.bn2 = norm_layer(in_channels=256, **({} if norm_kwargs is None else norm_kwargs))
            self.nin = nn.Dense(in_units=self.feat_dim, units=256,
                               weight_initializer=init.Normal(sigma=self.init_std))
            self.fc = nn.Dense(in_units=512, units=nclass,
                               weight_initializer=init.Normal(sigma=self.init_std))
...
def r2plus1d_resnet18_aetfc(nclass=4, pretrained=False,ctx=cpu(),**kwargs):
    model = R2Plus1D_AET2FC(nclass=nclass,**kwargs)
    if pretrained:
        pass

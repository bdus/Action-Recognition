    ...
    for i, batch in enumerate(val_data):
        ...
        for _, X in enumerate(data):
            ...
            if not opt.use_lateral:
                ...
                val_l2latel.append(latel[0])
                val_l2latel1.append(latel1[0])
            #pred = net(X.astype(opt.dtype, copy=False))
        ...
        if not opt.use_lateral:
            ...
            if opt.predict_T == 0: # normal
                ...
                loss0 = [loss_fn(yhat, y.astype(opt.dtype, copy=False)) for yhat, y in zip(val_outputs, label)]
                #loss1 = [loss_l2(xhat, takeT(x.astype(opt.dtype, copy=False)) )  for xhat, x in zip(l2out,data )]
        ...
        with ag.record():
            ...
            for _, X in enumerate(data):
                ...
                if not opt.use_lateral:
                    ...
                    x_hat = net1(latel[0].astype(opt.dtype, copy=False),
                             latel[1].astype(opt.dtype, copy=False),latel[2].astype(opt.dtype, copy=False))
                    l2out.append(x_hat)
                    l2latel.append(latel[0])
                    l2latel1.append(latel1[0])
                output.append(pred)
            ...
            if not opt.use_lateral:
                ...
                if opt.predict_T == 0: # normal
                    ...
                    loss2 = [loss_l2(f1,f2) for f1, f2 in zip(l2latel,l2latel1) ]
                    loss = [(l1+l2+l3) for l1, l2, l3 in zip(loss0,loss1,loss2)]
